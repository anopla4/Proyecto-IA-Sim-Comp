...
class TreatmentTree(object):
    ...
    def expand_selection(self, _posible_intervention: List[str], current_time) -> Node:
        ...
        for child in self._current._children:
            if (
                child.value in _posible_intervention
                and child.created_time == current_time
            ):
                _candidate_childs.append(child)
                _posible_intervention.remove(child.value)
        for intervention in _posible_intervention:
            self._node_count += 1
            self._current._children.append(
                Node(intervention, self._current, current_time, self.node_count)
            )
    ...
    def calculate_probability(self):
        ...
        def __calculate(nodes: List[Node]):
            ...
            for node in nodes:
                if node is not None:
                    node.probability_value = round(
                        (node.visit_count / node.parent.visit_count) * 100, 1
                    )
            childrens = []
            for node in nodes:
                if node is not None:
    ...
    def prunning(self, max_childs=2, acc_probability=75, low_level_exclusion=30):
        ...
        def best_childs(
            node: Node, max_childs, acc_probability, make_exclusion, exclusion
        ) -> List[Node]:
            childs = []
            acc_p = 0
            for child in node._children:
            ...
            if make_exclusion and i >= 1:
                ...
                prunning_childs.extend(
                    [n for n in childs[1:i] if n.probability_value > exclusion]
                )
            return prunning_childs if make_exclusion else childs[:i]

        def prunning_rec(
            nodes: List[Node], max_childs, acc_p, make_exclusion, exclusion
        ):
            if len(nodes) == 0:
                return
            for node in nodes:
                node._children = best_childs(
                    node, max_childs, acc_p, make_exclusion, exclusion
                )
            for node in nodes:
                prunning_rec(node._children, max_childs, acc_p, True, exclusion)

        prunning_rec(
            [self.root], max_childs, acc_probability, False, low_level_exclusion
        )
