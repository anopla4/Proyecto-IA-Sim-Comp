...
class FuzzyEngine:
    def __init__(
        self, increse_parameter: Parameter, decrese_parameter: Parameter
    ) -> None:
    ...
    def fuzzy_engine(self, env: Environment, rules: List[Rule]) -> Environment:
        ...
        for param in env.parameters:
            ...
            if len(destination_rules) > 0:
                param.value += self.__detect_variation(
                    fuzzing_state, param, destination_rules
                )
        return env

    def __detect_variation(
        self,
        fuzzing_state: Dict[str, Dict[str, float]],
        ...
        extended = r - l
        increse_value = self.__get_var_value(
            fuzzing_state, self._increse_parameter, increse_rules, extended
        )
        decrese_value = self.__get_var_value(
            fuzzing_state, self._decrese_parameter, decrese_rules, extended
        )
    ...
    def __get_var_value(
        ...
        rules_evaluation = []
        target_membresy_function = param_target.get_extended_membresy_functions(
            extended
        )
        for rule in rules:
            and_ = 1.1
    ...
    @staticmethod
    def defuzzing_by_centroid(
        agregation_function, lower_range: float, upper_range: float
    ):
        """
